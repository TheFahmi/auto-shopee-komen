    ...
    if (isset($responseJson['data']['message'][0]['msgs'])) {
        ...
        if (!empty($newMessages)) {
            ...
            $keywordsToResponses = [
                ...
                'spill' => 'Semua barang sesuai dengan gambar ya kak, silakan diorder kakak.',
                'perkenalkan' => 'Semua barang sesuai dengan gambar ya kak, silakan diorder kakak.',
                'introduce' => 'Semua barang sesuai dengan gambar ya kak, silakan diorder kakak.',
                'eta' => 'Semua barang sesuai dengan gambar ya kak, silakan diorder kakak.',
            ];

            // Check each keyword in the message and write the response if found
            foreach ($keywordsToResponses as $keyword => $response) {
                if (strpos(strtolower($message['content']), $keyword) !== false) {
                    file_put_contents('auto_reply.txt', $response);
                    break; // Stop checking after the first match
                } else {
                    file_put_contents('auto_reply.txt', 'Semua barang sesuai dengan gambar ya kak, silakan diorder kakak.');
                }
            }
        }
    }
